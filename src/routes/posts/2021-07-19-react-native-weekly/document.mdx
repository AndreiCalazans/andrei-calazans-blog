...
When it hits the limited capacity the write operation to the buffer is blocked until yu the read the output of the pipe.  In the current implementation of `BlobProvider.openFile` it doesn't read in parallel causing it to stay blocked forever if it hits the limited capacity. 
