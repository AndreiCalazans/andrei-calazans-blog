...
<h2 id="10-12-2023">
  <a href="#10-12-2023">12th October, 2023</a>
</h2>

Did you know that the way React Native Native Modules are implemented on Android makes the `getConstants` call synchronous in the JS thread?

Why does that matter? well, if you are doing something expensive during that step it can block your JS thread when you import that native module via NativeModules.\<your native module name\>.

This is based on an investigation on why our JS thread was blocked for 400ms.

After investigation we found it was [jail-monkey's getConstants call](https://github.com/GantMan/jail-monkey/blob/72ae35eeffec3c981b3497af364605e6e289da05/android/src/main/java/com/gantix/JailMonkey/JailMonkeyModule.java#L70-L82). Which is doing the Rooted device check within their getConstants call instead of relying on an asynchronous pattern.

I'll write a blogpost soon about this finding.

<h2 id="10-11-2023">
  <a href="#10-11-2023">10th October, 2023</a>
</h2>

Gosh, I have not written my technical dailies since February. I won't dive into why, but I have struggled
a bit with motivation after the second quarter of this year. But, I'm getting back on my feet.

I want to write about performance analysis with React Native since the material that covers all the topics in 
the React Native realm is scattered across the internet due to its cross-platform trait.

For instance, here are few links you could read to get a better understanding of profiling React Native apps:

- Profiling JavaScript you can learn from [Firefox Profiler Doc](https://profiler.firefox.com/docs/#/./guide-getting-started).
- [React Native JS runtime profiling](https://reactnative.dev/docs/profiling)
- [Profiling](https://developer.android.com/studio/profile/android-profiler) and [Tracing](https://developer.android.com/topic/performance/tracing) on Android.
- [Analyzing performance on iOS](https://developer.apple.com/documentation/xcode/analyzing-the-performance-of-your-metal-app)

The thing is, even if you read all of the above you would still need hands on practice with React Native to understand how it all ties together.
You will hear more from on this topic soon.

<h2 id="02-06-2023">
  <a href="#02-06-2023">6th February, 2023</a>
</h2>

I haven't written in a couple of days, but here is a blogpost out of something
I had to do at work today:

[How do you toggle the visibility of a component without losing its UI space?](/posts/2023-02-06/visible-component-react-native)

<h2 id="01-31-2023">
  <a href="#01-31-2023">31st January, 2023</a>
</h2>

Last day of the month is here!

**Android Cache**

I spend an awful amount of time trying to fix local compilation of an Android app today. Some commands that might help you:

You can clean your build cache by doing `./gradlew clean` in your Android folder. But sometimes this is not enough.

You can also purge global cache by `rm -rf ~/.gradle/caches/`.

Some recommend you also stop any running Gradle Daemons before you purge cache by doing: `./gradlew --stop`.

[This is a nice article on cleaning up all the generated cache from Android Studio](https://engineering.backmarket.com/put-your-android-studio-on-a-diet-fa4d364acb05).

In my case, turns out react-native-v8 outputs some build cache inside node_module so you might also need to make sure you purge any cache within node_modules.

I had to remove the build folder in v8 by `rm -rf node_modules/react-native-v8/android/build` before rerunning build. This what caused the issue I had.

**Random Links and Posts I Read Today**

[Blogpost on generative
AI](https://a16z.com/2022/11/17/the-generative-ai-revolution-in-games/) is
pretty good and fully agree on how generative AI reduces the cost of
development of games. This will enable a next wave of games.

[This post on how someone lost $209,640](https://www.mostlymetrics.com/p/i-lost-209640-of-my-own-money-trying?utm_source=profile&utm_medium=reader2) starting a business is a great story full of lessons, worth the read.

[This is nice introductory course on AWS and everything related it, hashtag CDK and Infra as Code](https://awsfundamentals.com/)

<h2 id="01-23-2023">
  <a href="#01-23-2023">23rd January, 2023</a>
</h2>

**Notes on building software**

[I saw this tweet](https://twitter.com/gordonphayes/status/1617535312313610247)
on someone advertising [Rive for animations](https://rive.app/) while comparing
its usage in a Flutter, SwiftUI, and React Native app.

One thing came to mind on how picking the framework to use does not impact
product quality. [I tweeted about this
too](https://twitter.com/Andrei_Calazans/status/1617622516406841344).

Than, What impacts product quality?

**User Experience**

As a user, I think apps often fail at providing their core feature a single step
away from you. Instead they add a bunch of things in the middle for no added
value.

Think a banking app, sending and receiving money should always be one press
away. This is often not true.

**Latency**

Latency is usually everywhere including in a framework you might choose,
however, from my personal experience, 99% of software latency is tipically found
in network requests.

How can network requests be slow?

There a multiple reasons but some might be:

1. A bad fetching strategy like waterfall requests.
2. No smart use of caching layers like CDNs.
3. Slow back-ends?

There are definitely more things to this topic.

<h2 id="01-20-2023">
  <a href="#01-20-2023">20th January, 2023</a>
</h2>

**A Non-obvious JS error**

Today I ran into a non-obvious error:

`null is not an object (evaluating 'iter[symbol.iterator]')`

This error happens when you try to spread an undefined value as if it was an Object or Array (or any other iterator).

Think this:

```
const someArray = remoteData.could?.be?.anArray;

return [...someArray]; // this will crash when someArray is undefined
```

While working with React Native, I had this crash and because the stack-trace
sucked it was a bit difficult to figure out where it came from.

<h2 id="01-19-2023">
  <a href="#01-19-2023">19th January, 2023</a>
</h2>

For the first time in my career I used "git bisect" to find an issue.

**Where it started**

While trying to fix something I noticed the screen I was working on had a broken
feature. At first I thought this was the result of my change, but it clearly was
unrelated.

When I looked at the Git history of where that screen's component was it had not
been changed for over a month. So this is indicative that whatever broke was
with one of its dependencies.

**Why git bisect would be useful?**

In this situation, I couldn't jump into every depedency imported within that
folder to find if any correlated code had been recently changed to try to spot
where the bug came from. And this is where "git bisect" shines.

Despite having never used "git bisect" I was quite aware of its feature and how it works.

**Using git bisect**

This is how I found the culprit code/commit.

1. Start by doing "git bisect start"
2. Git checkout to a commit about a month ago.
3. Test if the feature works.
4. If it did, do "git bisect good"
5. Now git bisect will checkout until another commit for you to test
6. if good you do "git bisect good" if bad you do "git bisect bad"

It will continue changing commits until it is down to zero revisions. At the end
it will log the commit which caused the regression.

And this is exactly how it worked for me.

**Manual vs. Automated testing**

I did the testing process fully manual since this was a UI feature that I did
not have a unit test for.

But if you can automate the check with a script you could have [Git bisect also
run](https://git-scm.com/docs/git-bisect#_bisect_run) that test for you and verify automatically, this is much faster.
